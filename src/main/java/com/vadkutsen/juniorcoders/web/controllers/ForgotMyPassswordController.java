...
import com.vadkutsen.juniorcoders.backend.service.*;
...
public class ForgotMyPassswordController {
    ...
    public String changeUserPasswordGet(@RequestParam("id") long id, @RequestParam("token") String token,
        ...
        if (LocalDateTime.now(Clock.systemUTC()).isAfter(passwordResetToken.getExpiryDate())) {
            ...
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME, "false");
            model.addAttribute(MESSAGE_ATTRIBUTE_NAME, i18NService.getMessage("resetPassword.token.expited", locale));
            return CHANGE_PASSWORD_VIEW_NAME;
        }
    ...
    public String changeUserPasswordPost(@RequestParam("principal_id") long userId,
        ...
        if (null == authentication) {
            ...
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME, "false");
            model.addAttribute(MESSAGE_ATTRIBUTE_NAME, "You are not authorized to pergorm this request.");
            return CHANGE_PASSWORD_VIEW_NAME;
        }

        User user =  (User) authentication.getPrincipal();
        if (user.getId() != userId) {
            LOG.error("Security breach! User {} is trying to make a password reset on behalf of {}", user.getId(), userId);
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME, "false");
            model.addAttribute(MESSAGE_ATTRIBUTE_NAME, "You are not authorized to pergorm this request.");
            return CHANGE_PASSWORD_VIEW_NAME;
        }
